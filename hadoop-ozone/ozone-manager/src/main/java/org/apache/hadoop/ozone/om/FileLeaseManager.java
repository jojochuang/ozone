...
import static org.apache.hadoop.ozone.om.OMConfigKeys.OZONE_OM_LEASE_RECHECK_INTERVAL_MS;
import static org.apache.hadoop.ozone.om.OMConfigKeys.OZONE_OM_LEASE_RECHECK_INTERVAL_MS_DEFAULT;
...
public class FileLeaseManager {
  public static final Logger LOG = LoggerFactory.getLogger(FileLeaseManager.class
          .getName());
  ...
  private final TreeMap<KeyIdentifier, Lease> leasesById = new TreeMap<>();

  private Daemon lmthread;
  private volatile boolean shouldRunMonitor;

  FileLeaseManager(OzoneManager ozoneManager, OzoneConfiguration conf) {
    this.ozoneManager = ozoneManager;
    this.keyManager = ozoneManager.getKeyManager();
    this.hardLimit = conf.getTimeDuration(OZONE_OM_FILE_LEASE_HARD_LIMIT,
        OZONE_OM_FILE_LEASE_HARD_LIMIT_DEFAULT, TimeUnit.MILLISECONDS);
    this.leaseRecheckIntervalMs = conf.getTimeDuration(
        OZONE_OM_LEASE_RECHECK_INTERVAL_MS,
        OZONE_OM_LEASE_RECHECK_INTERVAL_MS_DEFAULT, TimeUnit.MILLISECONDS);
    updateInternalLeaseHolder();
    loadLeaseFromOpenKeys();
    //startMonitor();
  }
  ...
  Collection<KeyIdentifier> getINodeIdWithLeases() {return leasesById.keySet();}
   ...
   */
  public synchronized Lease addLease(String holder, KeyIdentifier inodeId) {
  ...
  public void loadLeaseFromOpenKeys() {
    try {
      // find the list of open keys. For each of them add a lease.
      Map<String, List<OmKeyInfo>> clientWithOpenKeys =
          keyManager.listOpenKeys();

      clientWithOpenKeys.forEach((client, openKeys) -> {
        openKeys.forEach(
            keyInfo -> addLease(client, new KeyIdentifier(keyInfo)));
      });
    } catch (IOException e) {
      LOG.error("Unable to list open keys. Active leases may be lost.", e);
      throw new RuntimeException(e);
    }
  }

  synchronized void removeLease(KeyIdentifier inodeId) {
   ...
   */
  private synchronized void removeLease(
      Lease lease, KeyIdentifier inodeId) {
   ...
   */
  synchronized Lease reassignLease(
      Lease lease, INodeFile src,
      String newHolder) {
  ...
  class Lease {
    ...
    private final HashSet<KeyIdentifier> files = new HashSet<>();
    ...
    boolean removeFile(KeyIdentifier inodeId) {
    ...
    private Collection<KeyIdentifier> getFiles() {
  ...
  class Monitor implements Runnable {
    ...
    @Override
    public void run() {
      for(; shouldRunMonitor && shouldRun(); ) {
        try {
          // sleep now to avoid infinite loop if an exception was thrown.
          Thread.sleep(getLeaseRecheckIntervalMs());
  ...
  synchronized void checkLeases() {
    checkLeases(getExpiredCandidateLeases());
  }

  private synchronized void checkLeases(Collection<Lease> leasesToCheck) {
    ...
    for (Lease leaseToCheck : leasesToCheck) {
      ...
      final List<KeyIdentifier> removing = new ArrayList<>();
      ...
      Collection<KeyIdentifier> files = leaseToCheck.getFiles();
      ...
      for(KeyIdentifier id : files) {
        checkLease(leaseToCheck, removing, newHolder, id);
        continue;
        /*if (isMaxLockHoldToReleaseLease(start)) {
      ...
      for(KeyIdentifier id : removing) {
        removeLease(leaseToCheck, id);
      }
    }
  }

  private void checkLease(Lease leaseToCheck, List<KeyIdentifier> removing,
      String newHolder, KeyIdentifier id) {
    String p = "";
    try {
      KeyIdentifier keyIdentifier = null;
      if (keyManager.isDeleted(keyIdentifier)) {
        // INode referred by the lease could have been deleted.
        removeLease(keyIdentifier);
        return;
      }
      boolean completed = false;
      try {
        completed = releaseLease(leaseToCheck, p,
            newHolder);
      } catch (IOException e) {
  ...
  private boolean releaseLease(Lease leaseToCheck, String p,
